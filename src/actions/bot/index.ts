		...
		if (chatBotDomain) {
			...
			if (customerEmail) {
				...
				if (checkCustomer && !checkCustomer.customer.length) {
					...
					if (newCustomer) {
						...
						const response = {
							role: 'assistant',
							content: `Welcom aboard ${customerEmail.split('@')[0]}! I'm glad to connect with you. Is there anything you need help with?`
						};
          ...
          `,
				...
				if (chat[chat.length - 1].content.includes('(complete)')) {
					const firtUnansweredQuestion = await client.customerResponses.findFirst({
						where: {
					...
					if (firtUnansweredQuestion) {
						await client.customerResponses.update({
							where: {
								id: firtUnansweredQuestion.id,
							},
							data: {
								answered: message,
							}
