...
const BookAppointmentDate: FC<Props> = ({
	date,
	onBooking,
	onBack,
	register,
		...
		<div className="flex flex-col gap-5 justify-center">
			...
			<div className="flex gap-10 flex-col sm:flex-row">
				<div className="w-[300px]">
					...
					<CardDescription>
						During this call, we aim to explore potential avenues for partnership, promotional
						opportunities, or any other means through which we can contribute to the success of your
						company.
					</CardDescription>
				</div>
				<div>
					...
					{APPOINTMENT_TIME_SLOTS.map((slot, key) => (
						<Label htmlFor={`slot-${key}`} key={key}>
										...
										bookings.some(
											(booking) =>
												`${booking.date.getDate()}/${booking.date.getMonth()}` ===
													`${date?.getDate()}/${date?.getMonth()}` && booking.slot == slot.slot,
										)
										? 'bg-gray-300'
							...
							>
								<Input
									{...(bookings &&
									bookings.some((booking) => booking.date === date && booking.slot === slot.slot)
										? { disabled: true }
										: { disabled: false })}
									className="hidden"
									type="radio"
